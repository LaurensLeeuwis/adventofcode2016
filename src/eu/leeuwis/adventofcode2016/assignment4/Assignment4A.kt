...
private fun getEncryptedName(checksum: String, line: String, sector: Int): String {
    val encryptedName = line.substring(0, line.length - checksum.length - 2 - "${sector}".length)
    return encryptedName
}

private fun getSector(checksum: String, line: String): Int {
    val splitted = line.split("-");
    val lastPart = splitted[splitted.size - 1]
    val sector = lastPart.substring(0, lastPart.length - checksum.length - 2)
    return Integer.parseInt(sector)
}

private fun getChecksum(line: String) = line.split("[")[1].split("]")[0]

val givenInstructions = """gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
